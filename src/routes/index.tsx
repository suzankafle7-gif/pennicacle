...
function Home() {
  ...
  return (
    <div className="min-h-dvh bg-black text-white font-sans">
      {/* ─── Hero ── */}
      <section className="flex flex-col items-center justify-center px-6 py-32 text-center">
        ...
        <p className="text-xl md:text-2xl text-gray-400 max-w-2xl mb-12">
          Autonomous AI that ships production-ready code across your entire codebase.
        </p>
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-md flex flex-col gap-4">
          <input
            ...
            className="w-full px-4 py-3 rounded-lg bg-gray-900 border border-gray-800 focus:border-blue-500 outline-none transition-colors"
          />
          <button
            type="submit"
            disabled={status === "loading"}
            className="w-full px-4 py-3 rounded-lg bg-blue-600 hover:bg-blue-700 disabled:opacity-50 font-semibold transition-colors"
          >
      ...
      <section className="px-6 py-24 max-w-6xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {features.map((feature, i) => (
            <div key={i} className="p-8 rounded-2xl bg-gray-900/50 border border-gray-800">
              <div className="w-12 h-12 rounded-lg bg-blue-600/20 flex items-center justify-center text-blue-400 mb-6">
                {feature.icon}
              </div>
      ...
      {/* ─── Footer ── */}
      <footer className="px-6 py-12 text-center text-gray-600 border-t border-gray-900">
        <p>&copy; {new Date().getFullYear()} {businessName || "Pinnacle"}. All rights reserved.</p>
      </footer>
    </div>
  );
}
